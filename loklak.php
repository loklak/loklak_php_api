...
class Loklak {
	...
	public function account($name=null, $action=null, $data=null) {
		...
		if ($name) {
			$params = array('screen_name'=>$this->name);
			$request = Requests::request($this->requestURL, $headers, $params);
			if ($request->status_code == 200)
				return json_encode($request, true);
			else {
				$request = array();
		...
		else if ($this->action == "update" && $data) {
			...
			$request = array();
			$error = "No Query string has been given to run a query for account.	";
			$request['error'] = array_push($request, $error);
			return json_encode($request, true);
		}
	}
